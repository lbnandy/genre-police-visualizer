...
- Added Gentle, Standard, and Strong visual-response presets, an optional live FPS diagnostic, and playback frame-rate limits for Follow display, 120, 90, 60, and 30 FPS with deadline-based scheduling on high-refresh displays.
- Matched capsule, poster, and fullscreen canvas backing stores to their physical on-screen footprint and Windows display scaling, while retaining adaptive resolution reduction under sustained performance pressure.
- Reduced memory churn and repaint work by reusing per-frame spectrum geometry, skipping unchanged dynamic-style writes, and independently pacing broad themed backdrops; lightweight foreground and word-by-word lyric feedback now update at up to 60 FPS.
- Suspended rendering, audio analysis and local-model feeds, pointer hit testing, and adaptive backdrop sampling while the main window is hidden or minimized, with fresh system-audio capture and clean render timing when it becomes visible; active recording continues at full priority.
