...
- 降低复杂曲风的渲染开销，并根据持续性能压力自适应调整动态画布分辨率。
...
- Reduced rendering overhead in complex themes and added adaptive dynamic-canvas resolution under sustained performance pressure.
...
- 複雑なジャンル表現の描画負荷を軽減し、継続的な性能低下を検出した場合は動的キャンバスの解像度を自動調整するようにしました。
